...
const createDbPool = (prefix: string): Pool => {
  ...
  info(`Attempting to connect to ${prefix} DB: ${options.user}@${options.host}:${options.port}/${options.database}`);
...
export const testConnections = async (): Promise<void> => {
  ...
  try {
    ...
    warn(`Failed to connect to Replica database. Read operations might fail. Error: ${err.message}`);
